...
- ## 本地临时文件
	...
	- ### 本地临时文件会被清理
		- **本地临时文件** 产生后, 仅在 **当前小程序生命周期内** 保证有效, **小程序销毁后** 可能会被清理.
			- 参见: [[微信小程序 - 小程序生命周期]]
		- 若想将 **本地临时文件** 持久保存, 则可将 **本地临时文件** 转为 **本地缓存文件** 或 **本地用户文件** .
- ## 本地缓存文件 (不推荐使用)
	- ### 产生本地缓存文件
		- 调用 `FileSystemManager.saveFile()` 将 **本地临时文件** 转为 **本地缓存文件** .
			- ==已不推荐使用, 应使用功能更完整的 **本地用户文件**==
- ## 本地用户文件
	- ### 本地用户文件目录
		- 微信提供了一个 **本地用户文件** 目录, 开发者对这个目录有 **完全自由的读写权限** .
		- 用 `wx.env.USER_DATA_PATH` 可以获取到这个目录的路径.
			- 值为 `wxfile://usr` 或 `http://usr` .
	- ### 产生本地用户文件
		- 调用 `FileSystemManager.copyFile()` 将 **本地临时文件** 复制到 **本地用户文件** 目录下.
		  logseq.order-list-type:: number
		- 调用 `FileSystemManager` 的 `写文件` 相关 API , 直接创建 **本地用户文件** .
		  logseq.order-list-type:: number
- ## API 和 组件对各种文件的读写权限
	- | API 和 组件 | 读 | 写 |
	  | ---- | ---- | ---- |

		  ...
		  logseq.order-list-type:: number
			- 微信公众账号
			  logseq.order-list-type:: number
				- [[微信小程序]] / [[微信小游戏]] 
				  logseq.order-list-type:: number
					- 一般说 **小程序** , 可能会包含 **小游戏** , 可以认为 **小游戏** 是特殊的 **小程序** .
				- [[微信公众号]] / [[微信服务号]]
				  logseq.order-list-type:: number
					- 一般说 **公众号** , 可能会包含 **服务号** , 可以认为 **服务号** 是特殊的 **公众号** .
				- [[企业微信]]
				  logseq.order-list-type:: number
			- [[微信视频号]]
			  logseq.order-list-type:: number
		  ...
		  logseq.order-list-type:: number
			...
			- 主要是如下两种情况:
				...
				- [[微信消息与事件推送]]
				  logseq.order-list-type:: number
			- ==服务端开发与部署:==
				- 开发服务端程序, 自建服务器部署 (自己部署或订阅 **数据库** 和 **对象存储** 等服务), 通过公网地址访问.
				  logseq.order-list-type:: number
				- 开发云函数或服务端程序, 采用 [[腾讯云 CloudBase]] 部署 (提供 **数据库** 和 **对象存储** 等服务), 通过 [[微信云开发]] 相关 API 进行内网访问 (也可以配置域名) .
				  logseq.order-list-type:: number
				- 开发服务端程序, 采用 [[微信云托管]] 部署 (提供 **数据库** 和 **对象存储** 等服务), 通过 [[微信云托管]] 相关 API 进行内网访问 (也可以配置域名) .
				  logseq.order-list-type:: number
					- 其实  [[微信云托管]] 是复用了 [[腾讯云 CloudBase]] 的能力
- ## 接入微信能力的方式
	- 各种应用支持的接入方式:

	...
	- ### 冷启动情况
		- A 类情况: 启动场景不带 path .
		  logseq.order-list-type:: number
			- ==遵循重新启动策略 (见下文)==
			- ~~基础库小于 2.8.0 时, 情况不一样~~
				- 但无需关注, 因为参见 [用户使用的基础库版本分布](https://developers.weixin.qq.com/miniprogram/dev/framework/client-lib/version.html) 可知, 基础库版本小于 2.8.0 的用户几乎不存在.
		- B 类场景: 启动场景带有 path .
		  logseq.order-list-type:: number
			- ==进入 path 指向的页面.==
	- ### 重新启动策略
		...
		- 配置重启策略
		  logseq.order-list-type:: number
			- 配置 `app.json` 的 `window` 的 `restartStrategy` 属性
			  logseq.order-list-type:: number
